    ...
    <div className="flex h-screen">
      <div className="flex-1 max-w-5xl mx-auto p-6 flex flex-col">
        ...
        <div className="flex-grow overflow-y-auto space-y-6">
          {posts.map((post) => (
            <div key={post._id} className="border rounded-lg p-4 shadow-md bg-white">
              <div className="flex items-center mb-3">
                ...
                <p className="font-semibold text-lg">{post.user?.name || "Anonymous"}</p>
              </div>
              <p className="text-gray-700">{post.content}</p>
              <div className="flex items-center mt-3 space-x-4 text-gray-600">
                <div className="flex items-center space-x-1 cursor-pointer" onClick={() => handleLike(post._id)}>
